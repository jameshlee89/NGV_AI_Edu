...
| 기법 | 설명 | 사용 시점 |
|---|---|---|
| 동등분할 (Equivalence Partitioning) | 입력을 유효/무효 클래스로 나누고 각 클래스의 대표값으로
  검증 | 입력 도메인이 넓을 때 |
| 경계값분석 (Boundary Value Analysis) | 최소/최대/0/빈 값 등 경계 지점을 검증 | 범위·길이 제한이
  있는 입력 |
| 오류추정 (Error Guessing) | 경험상 자주 발생하는 실수(None, 빈 문자열, 중복 호출 등)를 노려서
  검증 | 방어적 코드(§ 오류 처리) 검증 |
| 상태전이 테스트 (State Transition Testing) | 상태 머신의 상태·전이·가드를 검증 | 상세설계서 8장
  (상태전이 상세) 구현 검증 |
| 결정테이블 테스트 (Decision Table Testing) | 정책 의사결정표의 조건 조합별 기대 동작을 검증 |
  상세설계서 7장(정책 의사결정표) 구현 검증 |
| 버그 재현 (Regression) | 실제 발견된 버그를 재현하는 실패 테스트를 먼저 작성 | 버그 수정 시 |
| 실제 컴포넌트 기반 (Real Component) | 모의 객체 없이 실제 의존 대상으로 검증 | 기본 선택지 —
  `writing-good-tests.md`에 따라 가능하면 항상 이 방식을 우선한다 |
| 통합형 단위 테스트 (Integration-style) | 모의 처리 범위를 최소화하고 여러 실제 컴포넌트를 함께
  실행 | 모의 객체 준비가 테스트 로직보다 커질 때 |
